/*
Name            : Omkar Ashok Sawant
Date            : 18/11/2025
Description     : Quick Sort is a Divide and Conquer algorithm. 
                  It picks an element as pivot and partitions the given array around the picked pivot
Requirements    : 1.Prompt the user to Enter the length of the array(Size) and Declare an integer array 
                  2.Prompt the user to Enter the  elements of the array  and read it using scanf()
                  3.Call quick_sort function to sort the elements of the array
                  4.Use the partition function to find the right position of pivot element
                  5.Print the sorted element of the array
Input	        : 5 4 3 2 1
Output	        : 1 2 3 4 5
*/

#include "main.h"

/* Main Function */
int main()
{
	int i, limit, option;

	/* Scanning the size */
// 	printf("Enter the size of an array\n");
	scanf("%d", &limit);

	/* Declaring the array with size */
	int arr[limit];

		
// 	printf("Enter the array elements\n");
	for (i = 0; i < limit; i++)
	{
	    /* Scanning the array elements */
		scanf("%d", &arr[i]);
	}
	
	/* Calling the function to bubble sort */
	quick_sort(arr, 0, limit - 1);
	
	printf("Sorted array is : ");
	for (i = 0; i < limit; i++)
	{
	    printf("%d ", arr[i]);
	}
		
	return 0;
}
